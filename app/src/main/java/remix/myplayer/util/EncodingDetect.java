 ...
 *
 * EncodingDetect.java<br> �Զ���ȡ�ļ��ı���
 *
 * @author Billows.Van
 * @version 1.0
...
public class EncodingDetect {
  ...
  static class BytesEncodingDetect extends Encoding {
     ...
     * enumeration (GB2312, HZ, BIG5, EUC_TW, ASCII, or OTHER) Description: This function looks at the
     * URL contents and assigns it a probability score for each encoding type. The encoding type with
    ...
    public int detectEncoding(URL testurl) {
      ...
      try {
        ...
        while ((bytesread = chinesestream.read(rawtext, byteoffset, rawtext.length - byteoffset))
            > 0) {
    ...
    /**
     * Function : detectEncoding Aruguments: File Returns : One of the encodings from the Encoding
     * enumeration (GB2312, HZ, BIG5, EUC_TW, ASCII, or OTHER) Description: This function looks at the
     * file and assigns it a probability score for each encoding type. The encoding type with the
    ...
    /**
     * Function : detectEncoding Aruguments: byte array Returns : One of the encodings from the
     * Encoding enumeration (GB2312, HZ, BIG5, EUC_TW, ASCII, or OTHER) Description: This function
     * looks at the byte array and assigns it a probability score for each encoding type. The encoding
     * type with the highest probability is returned.
     */
          ...
          if (rawtext[i + 1] == '{') {
            ...
            while (i < rawtextlen - 1) {
              if (rawtext[i] == 0x0A || rawtext[i] == 0x0D) {
                ...
                i++;
                break;
              } else if ((0x21 <= rawtext[i] && rawtext[i] <= 0x77) && (0x21 <= rawtext[i + 1]
                  && rawtext[i + 1] <= 0x77)) {
                hzchars += 2;
                row = rawtext[i] - 0x21;
              ...
              } else if ((0xA1 <= rawtext[i] && rawtext[i] <= 0xF7) && (0xA1 <= rawtext[i + 1]
                  && rawtext[i + 1] <= 0xF7)) {
                hzchars += 2;
                row = rawtext[i] + 256 - 0xA1;
                column = rawtext[i + 1] + 256 - 0xA1;
    ...
    /**
     * Function: big5_probability Argument: byte array Returns : number from 0 to 100 representing
      ...
      for (i = 0; i < rawtextlen - 1; i++) {
        ...
        if (rawtext[i] >= 128) {
          ...
          if (0xA1 <= rawtext[i] && rawtext[i] <= 0xF9 && // Original Big5 range
              ((0x40 <= rawtext[i + 1] && rawtext[i + 1] <= 0x7E) || (0xA1 <= rawtext[i + 1]
                  && rawtext[i + 1] <= 0xFE))) {
            bfchars++;
            totalfreq += 500;
            row = rawtext[i] - 0xA1;
            ...
            if (Big5Freq[row][column] != 0) {
              ...
              ((0x40 <= rawtext[i + 1] && rawtext[i + 1] <= 0x7E) || (0x80 <= rawtext[i + 1]
                  && rawtext[i + 1] <= 0xFE))) {
            bfchars++;
            totalfreq += 500;
            row = rawtext[i] - 0x81;
        ...
        if (rawtext[i] == (byte) 0x1B && i + 3 < rawtextlen) { // Escape char ESC
          if (rawtext[i + 1] == (byte) 0x24 && rawtext[i + 2] == 0x29
            ...
            while (rawtext[i] != (byte) 0x1B) {
              dbchars++;
              if ((0x21 <= rawtext[i] && rawtext[i] <= 0x77) && (0x21 <= rawtext[i + 1]
                  && rawtext[i + 1] <= 0x77)) {
                isochars++;
                row = rawtext[i] - 0x21;
                column = rawtext[i + 1] - 0x21;
            ...
            while (rawtext[i] != (byte) 0x1B) {
              dbchars++;
              if ((byte) 0x21 <= rawtext[i] && rawtext[i] <= (byte) 0x7E && (byte) 0x21 <= rawtext[i
                  + 1]
      ...
      for (i = 0; i < rawtextlen; i++) {
        if ((rawtext[i] & (byte) 0x7F) == rawtext[i]) { // One byte
          ...
          i++;
            ...
            && // Three bytes
            i + 2 < rawtextlen && -128 <= rawtext[i + 1] && rawtext[i + 1] <= -65 && -128 <= rawtext[i
            + 2]
            && rawtext[i + 2] <= -65) {
          goodbytes += 3;
          i += 2;
      ...
      for (i = 0; i < rawtextlen - 1; i++) {
        ...
        if (rawtext[i] >= 0) {
          ...
          if ((byte) 0x81 <= rawtext[i]
            ...
            krchars++;
            totalfreq += 500;
            if ((byte) 0xA1 <= rawtext[i] && rawtext[i] <= (byte) 0xFE && (byte) 0xA1 <= rawtext[i
                + 1]
                && rawtext[i + 1] <= (byte) 0xFE) {
              row = rawtext[i] + 256 - 0xA1;
              column = rawtext[i + 1] + 256 - 0xA1;
