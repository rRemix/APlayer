...
fun BlackListLogic() {
  ...
  val blackList = settingState.common.blacklist
      ...
      if (result.resultCode == Activity.RESULT_OK) {
        result.data?.data?.let { uri ->
          ...
          if (folder?.isDirectory == true) {
            ...
            if (folderPath != null) {
              settingVM.setBlacklist(blackList.toMutableSet().apply {
                add("${Environment.getExternalStorageDirectory()}/$folderPath")
              })
  ...
  NormalDialog(
    ...
    onPositive = {
      settingVM.setBlacklist(emptySet())
