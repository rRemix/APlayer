...
public class DBContentProvider extends ContentProvider {
  ...
  public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
    ...
    try {
      ...
      cursor = db
          .query(match == PLAY_LIST_MULTIPLE ? PlayLists.TABLE_NAME : PlayListSongs.TABLE_NAME,
              projection, selection, selectionArgs, null, null, null);
      cursor.setNotificationUri(App.getContext().getContentResolver(),
          Uri.parse(CONTENT_AUTHORITY_SLASH));
    } catch (Exception e) {
      e.printStackTrace();
    } finally {
  ...
  public Uri insert(Uri uri, ContentValues values) {
    ...
    try {
      if (match == PLAY_LIST_MULTIPLE || match == PLAY_LIST_SONG_MULTIPLE) {
        long rowId = db
            .insert(match == PLAY_LIST_MULTIPLE ? PlayLists.TABLE_NAME : PlayListSongs.TABLE_NAME,
                null, values);
        if (rowId > 0) {
          newUri = ContentUris.withAppendedId(
              match == PLAY_LIST_MULTIPLE ? PlayLists.CONTENT_URI : PlayListSongs.CONTENT_URI,
              rowId);
          App.getContext().getContentResolver().notifyChange(newUri, null
/**match == PLAY_LIST_MULTIPLE ? mPlayListObserver : mPlayListSongObserver*/);
        }
      }
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public int delete(Uri uri, String selection, String[] selectionArgs) {
    ...
    try {
      deleteRow = db
          .delete(match == PLAY_LIST_MULTIPLE ? PlayLists.TABLE_NAME : PlayListSongs.TABLE_NAME,
...
/**match == PLAY_LIST_MULTIPLE ? mPlayListObserver : mPlayListSongObserver*/);
  ...
  public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
    ...
    try {
      updateRow = db
          .update(match == PLAY_LIST_MULTIPLE ? PlayLists.TABLE_NAME : PlayListSongs.TABLE_NAME,
